...
class RAGPipeline:
    ...
    def build_from_documents(self, pdf_path: str, chunk_size: int = 512):
        ...
        self.embed_manager = EmbeddingManager(
            llm_provider=self.llm_provider, api_key=self.api_key
        )
        ...
        self.retriever = Retriever(
            self.index, top_k=initial_top_k, use_hybrid=self.use_hybrid
        )
    ...
    def query(
        ...
        if self.generator is None:
            raise ValueError(
                "Pipeline not initialized. Call build_from_documents() or load_existing() first."
            )
